<template>
  <div class="hero-page">
    ...
    <div class="univers col-12">
      ...
      <div class="card-container">
        <UniversCard
          v-for="univers in universList"
          :key="univers"
          :univers="univers"
          :imageSrc="`/path/to/univers/images/${univers.toLowerCase()}.jpg`"
          :isSelected="selectedUnivers === univers"
          @select-univers="selectUnivers"
        />
      </div>
    </div>
    <button @click="submitSelection" class="btn btn-light mt-3">Let's Play</button>
  </div>
</template>
